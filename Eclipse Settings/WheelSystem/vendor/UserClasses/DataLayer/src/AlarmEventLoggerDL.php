...
class AlarmEventLoggerDL extends DatabaseManager implements DatabaseFunctionsInt
{
    ...
    public function update(array $data):bool
    {
        $connector=$this->dbConnect();  //connect to MariaDB database
        if(isset($connector)){
            /***UPDATE Alarm Event Data in Database*******/
            $query="UPDATE AlarmEventLog SET ";
            $query.="AlarmCause=?,DefectSize=?,PredictedDate=?";
            ...
            $predictedDate=$data["predictedWheelFailureDate"];            
            $measurementID=$data["measurementID"];            
            $stmt->bind_param("sdss",$alarmCause,$defectSize,$predictedDate,$measurementID);
            $status_message=$stmt->execute();
    ...
    public function searchData(array $data):array
    {
        ...
        if(isset($connector)){
            ...
            if($status){
                ...
                $query.="WHERE (AlarmEventLog.RefDate>=? AND AlarmEventLog.RefDate<=?) ";
                $query.="AND (WheelMeasurements.MeasurementID=AlarmEventLog.MeasurementID) AND ";
                $query.="WheelMeasurements.MeasurementID NOT IN ".$IN_STRING;
                $stmt=$connector->prepare($query);               
                $stmt->bind_param("ss",$alarmStartDate,$alarmEndDate);
                $status=$stmt->execute();
                $stmt->store_result();
                $stmt->bind_result($measurementID,$coachNumber,$setNumber,$axleNumber,$wheelID,$operatorName,$measDate,$measTime,$Sh,$qR,$FW,$H,$refID,$measID,$alarmCause,$defectSize,$refDate,$predictedDate);
                if($status){
                    while($stmt->fetch()){
                        $arr["measurementID"]=$measurementID;
                        $arr["spreadRim"]="";
