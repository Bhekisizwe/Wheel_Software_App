...
if(isset($_SESSION["staffNumber"])){
    ?>
        ...
        <title>View Wheel Measurements Alarm Events</title>
        </head>
        <body style="background-color:#ccccff">
        <div class="containter-fluid" align="center">
        	<h3 class="h3">View Wheel Measurements Alarm Events</h3>
            	...
            	<form style="border:1px solid #888888;width:35%;background-color:#eeeeee" class="rounded" id="searchForm">
            		<label>Wheel Measurements Alarms Search Start Date:</label>
            		<input type="date" id="startDate" class="form-control" value="" min="2010-01-01" max="2110-01-01" required><p>  		
            		
    				<label>Wheel Measurements Alarms Search End Date:</label>
    				<input type="date" id="endDate" class="form-control" value="" min="2010-01-01" max="2110-01-01" required><p>
    				<p>
    				<input type="submit" id="viewButton" class="btn btn-primary" value="SEARCH FOR ALARM EVENTS">
            	</form></div><p> 
            ...
            $("#searchForm").on("submit",function(event){				
			    ...
			    if($("#searchForm")[0].checkValidity()){
			    	...
			    	$.ajax({
						...
						url: "/alarmevent/"+startDate+"_"+endDate,							
						dataType: 'json',
						cache: false,							
						success: function (data) {
							$("#viewButton").val("SEARCH FOR ALARM EVENTS");							
							if(data[0]["transactionStatus"]){	
								var html_str="";
								html_str+="<table class='table table-striped table-light table-sm' style='width:100%'>";
								html_str+="<th>CoachNo.</th><th>WheelID</th>";	
								html_str+="<th>Sh(mm)</th><th>FW(mm)</th><th>qR(mm)</th><th>H(mm)</th>";
								html_str+="<th>Spread Rim(mm)</th><th>Wheel Skid(mm)</th><th>Cut Tyre Depth(mm)</th><th>Cut Tyre Width(mm)</th><th>Cut Tyre Dist. to Flange(mm)</th>";
								html_str+="<th>Reference Date</th><th>Failure Date</th><th>Defect Size(mm)</th><th>Root Cause of Alarm</th>";						
								for(var i=0;i<data.length;i++){									
									html_str+="<tr><td>"+data[i]["coachNumber"]+"</td>";
									html_str+="<td>"+data[i]["wheelID"]+"</td>";																		
									html_str+="<td>"+data[i]["flangeHeight"]+"</td>";
									html_str+="<td>"+data[i]["flangeWidth"]+"</td>";
									html_str+="<td>"+data[i]["toeCreep"]+"</td>";
									html_str+="<td>"+data[i]["hollowing"]+"</td>";
									html_str+="<td>"+data[i]["spreadRim"]+"</td>";
									html_str+="<td>"+data[i]["wheelSkid"]+"</td>";
									html_str+="<td>"+data[i]["cutTyreDepth"]+"</td>";
									html_str+="<td>"+data[i]["cutTyreWidth"]+"</td>";
									html_str+="<td>"+data[i]["cutTyreDistanceFromFlange"]+"</td>";
									html_str+="<td>"+data[i]["referenceDate"]+"</td>";
									html_str+="<td>"+data[i]["predictedWheelFailureDate"]+"</td>";
									html_str+="<td>"+data[i]["defectSize"]+"</td>";
									html_str+="<td>"+data[i]["alarmCause"]+"</td></tr>";
								}
								html_str+="</table><p>";
								//alert(html_str);
								if(data[0]["coachNumber"]!=""){
									//alert(html_str);
