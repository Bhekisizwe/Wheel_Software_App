        ...
        <div class="containter-fluid" align="center">
        	<h3 class="h3">Edit User Role</h3>
    			...
    			<form style="border:1px solid #888888;width:40%;background-color:#eeeeee" class="rounded" id="roleForm">
		    		...
		    		<input type="submit" id="editRole" class="btn btn-primary" value="EDIT USER ROLE ACCESS RIGHTS"><p>	
        ...
        $(document).ready(function(){             
		    ...
		    function(data,status){    			    	
		    	...
		    	var html_str="<table style='width:100%'><th align='center'>Activity Name</th><th align='center'>Create</th><th align='center'>Read</th><th align='center'>Update</th><th align='center'>Delete</th>";
   		    	for(var i=0;i<data['activityRights2DArray'].length;i++){
   			    	if(data['activityRights2DArray'][i]['activityName']!="Planning Report Management"){			    				    	
       		    		html_str+="<tr><td align='left'>"+data['activityRights2DArray'][i]['activityName']+"<input type='hidden' id='activityName"+i+"' value='"+data['activityRights2DArray'][i]['activityID']+"'><input type='hidden' id='accessName"+i+"' value=''></td>";
   		    	...
   		    	html_str+="<tr><td align='left'>"+data['activityRights2DArray'][j]['activityName']+"<input type='hidden' id='activityName"+j+"' value='"+data['activityRights2DArray'][j]['activityID']+"'><input type='hidden' id='accessName"+j+"' value=''></td>";
		    	...
		    	for(var i=0;i<data["columnVisibility2DArray"].length;i++){        			    		    				    	
		    		html_str+="<tr><td align='left'>"+data['columnVisibility2DArray'][i]['columnName']+"<input type='hidden' id='columnName"+i+"' value='"+data['columnVisibility2DArray'][i]['columnID']+"'><input type='hidden' id='planningName"+i+"' value=''></td>";
					...
					url="/userrole/"+roleid;            
		   		    ...
		   		    function(data,status){    			    	
		   		    	...
		   		    	for(var i=0;i<data["activityRights2DArray"].length;i++){		   		    		
							for(var m=0;m<rightsArray.length;m++){																								
								if(data["activityRights2DArray"][i]["activityID"]==rightsArray[m]["activityID"]){
									...
									$("#accessName"+m).val(data["activityRights2DArray"][i]["accessID"]);																
				    	...
				    	for(var j=0;j<data["columnVisibility2DArray"].length;j++){        			    		    				    	
				    		html_str+="<tr><td align='left'>"+data['columnVisibility2DArray'][j]['columnName']+"<input type='hidden' id='planningName"+j+"' value='"+data['columnVisibility2DArray'][j]['planningID']+"'><input type='hidden' id='columnName"+j+"' value='"+data['columnVisibility2DArray'][j]['columnID']+"'></td>";
				...
				if(n>0){
					if($("#roleForm")[0].checkValidity()){						
						...
						for(var k=0;k<rightsArray.length;k++){							
							...
							//if(str.length>0){    							
								buildObjLiteral[""+counter+""]={'activityID':$('#activityName'+k).val(),'accessID':$('#accessName'+k).val(),'activityRights':str};    							
								counter++;
							//}
						}
						...
						if($("#"+generated_id+":checked").length>0){
    						...
    						for(var n=0;n<columnArray.length;n++){        						   							
    							buildObjLiteral[""+n+""]={'columnID':$('#columnName'+n).val(),'planningID':$('#planningName'+n).val(),'columnVisibility':$('input[name=columnName'+n+'Visible]:checked').val()};
    						}
						...
						if($("#editRole").length>0){
							$('#editRole').val("UPDATING USER ROLE ACCESS RIGHTS");
							$('#editRole').prop('disabled', true);
							//we are updating user role
							//updating user role access rights
