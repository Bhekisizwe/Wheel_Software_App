        ...
        $(document).ready(function () {
            ...
            $("#importForm").submit(function (event) {
        		...
        		if($('#importForm')[0].checkValidity()){
        			$("#uploadButton").val("UPLOADING ASSET REGISTER");
                    $("#uploadButton").prop("disabled", true);

                    $.ajax({
                        type: "POST",
                        enctype: 'multipart/form-data',
                        url: "/assetregisterservice/import/",
                        data: data,
                        processData: false,
                        contentType: false,
                        cache: false,
                        ...
                        success: function (data) {
                        	$("#uploadButton").val("UPLOAD ASSET REGISTER");
                            $("#uploadButton").prop("disabled", false);
                           	if(data["transactionStatus"]){
								alert("Successfully Imported the Asset Register!");
                           	}
                           	else{
								...
								else if(data["errorAssocArray"]["errorCode"]=="0x08"){
									alert(data["errorAssocArray"]["errorDescription"]);
								}
								else if(data["errorAssocArray"]["errorCode"]=="0x06"){
									alert(data["errorAssocArray"]["errorDescription"]);
								}
								else if(data["errorAssocArray"]["errorCode"]=="0x02"){
									alert(data["errorAssocArray"]["errorDescription"]);
								}
						    	else{								    	
