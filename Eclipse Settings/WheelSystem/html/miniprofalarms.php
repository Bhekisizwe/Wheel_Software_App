    ...
    ?>
        ...
        <div class="containter-fluid" align="center">
        	<h3 class="h3">Manage Wheel Measurements Alarm Settings</h3>
    			...
    			<form style="border:1px solid #888888;width:65%;background-color:#eeeeee" class="rounded" id="alarmForm">    				
        		    	...
        		    	if(data["transactionStatus"]){
        					if(data["warning2DArray"]!=null && data["alarms2DArray"]!=null){
        						...
        						for(var j=0;j<data["alarms2DArray"].length;j++){
									$("#norminalLevel"+j).val(data["alarms2DArray"][j]["norminalLevel"]);
									$("#alarmLevel"+j).val(data["alarms2DArray"][j]["alarmLevel"]);									
        						}
			...
			$("#alarmForm").on("submit",function(event){
				...
				for(var i=0;i<alarmArray.length;i++){
					...
					case "Flange Height":
						...
						if(Number($("#warningLevel"+i).val())<=Number($("#alarmLevel"+i).val())){
							alert("Cannot have Flange Height warning threshold less than or equal to alarm threshold");
							$("#warningLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						if(Number($("#alarmLevel"+i).val())<=Number($("#norminalLevel"+i).val())){
							alert("Cannot have Flange Height alarm threshold less than or equal to norminal threshold");
							$("#norminalLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						break;
					case "Flange Width":
						...
						if(Number($("#warningLevel"+i).val())>=Number($("#alarmLevel"+i).val())){
							alert("Cannot have Flange Width warning threshold greater than or equal to alarm threshold");
							$("#warningLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						if(Number($("#alarmLevel"+i).val())>=Number($("#norminalLevel"+i).val())){
							alert("Cannot have Flange Width alarm threshold greater than or equal to norminal threshold");
							$("#norminalLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						break;
					case "Hollowing":
						//Greater Than or Equal to
						if(Number($("#warningLevel"+i).val())<=Number($("#alarmLevel"+i).val())){
							alert("Cannot have Hollowing warning threshold less than or equal to alarm threshold");
							$("#warningLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						if(Number($("#alarmLevel"+i).val())<=Number($("#norminalLevel"+i).val())){
							alert("Cannot have Hollowing alarm threshold less than or equal to norminal threshold");
							$("#norminalLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						break;
					case "Toe Creep":
						//Less Than	or Equal to	
						if(Number($("#warningLevel"+i).val())>=Number($("#alarmLevel"+i).val())){
							alert("Cannot have Toe Creep warning threshold greater than or equal to alarm threshold");
							$("#warningLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}
						if(Number($("#alarmLevel"+i).val())>=Number($("#norminalLevel"+i).val())){
							alert("Cannot have Toe Creep alarm threshold greater than or equal to norminal threshold");
							$("#norminalLevel"+i).val("");
							$("#alarmLevel"+i).val("");
						}												
					}
				...
				if($("#alarmForm")[0].checkValidity()){						
					...
					for(var j=0;j<alarmArray.length;j++){
						...
						objData["alarms2DArray"][""+j+""]={"settingsID":$("#settingsID"+j).val(),"coachID":$("#coachTypes").val(),"alarmLevel":$("#alarmLevel"+j).val(),"norminalLevel":$("#norminalLevel"+j).val()};
					}		
																				
					json_data=JSON.stringify(objData);	
					//alert(json_data);					
					if($("#addAlarms").length>0){
						$('#addAlarms').val("SETTING WHEEL ALARM THRESHOLDS");
						$('#addAlarms').prop('disabled', true);
						//we are assigning
						//assign number of axles
						$.ajax({
						type: "POST",
						contentType: "application/json",
						url: "/wheelalarmsettings/add",	
						data:json_data,						
						dataType: 'json',
						cache: false,							
						success: function (data) { 
							$('#addAlarms').val("SET WHEEL ALARM THRESHOLDS");
							$('#addAlarms').prop('disabled', false);							               							
							if(data["transactionStatus"]){
								alert("Successfully Set Wheel Alarm Thresholds!");								
								location.reload();													
							}
							else{												
								if(data["errorAssocArray"]["errorCode"]=="0x19"){
					...
					if($("#editAlarms").length>0){
						$('#editAlarms').val("UPDATING WHEEL ALARM THRESHOLDS");
						$('#editAlarms').prop('disabled', true);
						//we are assigning
						//assign number of axles
						$.ajax({
						type: "POST",
						contentType: "application/json",
						url: "/wheelalarmsettings/update",	
						data:json_data,						
						dataType: 'json',
						cache: false,							
						success: function (data) { 
							$('#editAlarms').val("EDIT WHEEL ALARM THRESHOLDS");
							$('#editAlarms').prop('disabled', false);							               							
							if(data["transactionStatus"]){
								alert("Successfully Updated Wheel Alarm Thresholds!");								
								location.reload();													
							}
							else{												
								if(data["errorAssocArray"]["errorCode"]=="0x19"){
